...
/*
	...
	State names: 'A', 'B', 'C', ...
*/
void print_dfa_table(int tab[][SYMBOLS], /* DFA table */
...
*/
int get_optimized_DFA(char stnt[][STATES + 1], int n, int dfa[][SYMBOLS],
					  int n_sym, int newdfa[][SYMBOLS]) {
...
*/
int set_new_equiv_class(char stnt[][STATES + 1], int n, int newdfa[][SYMBOLS],
						int n_sym, int n_dfa) {
	int i, j, k;
	for (i = 0; i < n; i++) {
		for (j = 0; j < n_sym; j++) {
...
*/
int optimize_DFA(int dfa[][SYMBOLS],	  /* DFA state-transition table */
...
*/
void get_NEW_finals(char *newfinals,		 /* new DFA finals */
...
void main() {
	...
	N_optDFA_states = optimize_DFA(DFAtab, N_DFA_states, N_symbols, DFA_finals,
								   StateName, OptDFA);
